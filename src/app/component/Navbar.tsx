        ...
        <div className="md:hidden">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-white shadow-md">
            ...
            <div className="relative">
              ...
              >
                Specialties
                <ChevronDown className={`h-4 w-4 transition-transform duration-300 ${isSpecialtiesOpen ? 'rotate-180' : ''}`} />
              </button>
              <div className={`pl-4 space-y-1 transition-all duration-300 ease-in-out ${isSpecialtiesOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
                {specialties.map((specialty, index) => (
                  <Link
                    key={index}
                    href={`/specialties/${specialty.toLowerCase().replace(/\s+/g, '-')}`}
